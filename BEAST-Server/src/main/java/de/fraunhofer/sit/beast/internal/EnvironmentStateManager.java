...
public class EnvironmentStateManager {
	...
	private static final Logger LOGGER = LogManager.getLogger(EnvironmentStateManager.class);

	public static void setEnvironment(IDevice d, DeviceState stateAfterEnvironmentSet, String environment,
			boolean reset) throws SQLException {
		long msBefore = System.currentTimeMillis();
		LOGGER.info(
				String.format("Restoring environment of %s: %s", d.getDeviceInfo().getLongIdentifier(), environment));
		...
		LOGGER.info(String.format("Restoring environment of %s (%s) completed, took %d ms",
				d.getDeviceInfo().getLongIdentifier(), environment, System.currentTimeMillis() - msBefore));
	...
	public static synchronized void saveEnvironmentState(IDevice dev, String saveName) throws SQLException {
		...
		SavedEnvironment p = Database.INSTANCE.getSavedEnvironmentUnsafe(dev.getDeviceInfo().ID,
				SAVING_IN_PROGRESS_STATE);
		if (p != null) {
		...
		env.name = saveName;
		daoEnv.update(env);
		LOGGER.info(String.format("Environment saved of %s: %s, took %d ms", dev.getDeviceInfo().getLongIdentifier(),
				saveName, System.currentTimeMillis() - start));
	}
	...
	public static void loadEnvironmentState(IDevice dev, String environment) throws SQLException {
		setEnvironment(dev, dev.getDeviceInfo().state, environment, false);
	}
